...
```groovy
dependencies {
    implementation 'com.deepgram:deepgram-java-sdk:0.10.0' // x-release-please-version
}
```
...
- **`sendConfigure(...)`** adjusts the speech-rate multiplier mid-stream. Speeds range from `0.5` to `1.5` in `0.05` steps. The server replies via `onConfigureSuccess` or a typed `onConfigureFailure` (e.g. `SPEED_OUT_OF_RANGE`).
...
try {
    String json = DeepgramJson.MAPPER.writeValueAsString(request);
    ...
    CreateKeyV1Response parsed = DeepgramJson.MAPPER.readValue(payload, CreateKeyV1Response.class);
