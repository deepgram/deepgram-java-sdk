...
- Fern 4.21.0 now generates `AutoCloseable` root clients and ownership-aware OkHttp cleanup. The permanent custom client wrappers inherit this behavior; do not add wrapper-local lifecycle code. `ClientOptions.java` remains temporarily frozen only for its Deepgram header/version markers, and reconciliation must retain Fern's lifecycle code.
