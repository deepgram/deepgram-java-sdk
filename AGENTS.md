...
- `src/main/java/com/deepgram/resources/speak/v2/websocket/V2WebSocketClient.java` - forward-compat patch. Fern's generated `handleIncomingMessage` dispatcher routes any unrecognized message type to `onError` with "Update your SDK version...", which makes a benign new server control frame look fatal to a deployed client. Patched so the unrecognized-type branch is a no-op — the raw frame is already delivered via `onMessage(String)` earlier in the method, so consumers still see it. Mirrors the JS/Python SDKs' forward-compat behavior and is regression-guarded by `src/test/java/com/deepgram/SpeakV2ForwardCompatTest.java`. Use the standard `.bak` swap/restore workflow during regen review; re-apply the no-op after regen, and unfreeze once the generator stops treating unknown frames as errors. **Note:** the listen v2 client (`resources/listen/v2/websocket/V2WebSocketClient.java`) has the identical generated bug and is a candidate for the same patch in a follow-up.
