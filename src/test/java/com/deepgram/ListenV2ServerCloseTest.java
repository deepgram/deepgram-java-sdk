...
/** Regression coverage for server-initiated Flux stream closes. */
class ListenV2ServerCloseTest {
    ...
    void setUp() throws Exception {
        ...
        Environment environment = Environment.custom()
                ...
                .agentRest(base)
                .build();
        client = DeepgramClient.builder().apiKey("test").environment(environment).build();
    }
    ...
    void serverInitiatedCloseNotifiesClient() throws Exception {
        ...
        try {
            ws.connect(V2ConnectOptions.builder().model(ListenV2Model.FLUX_GENERAL_EN).build())
    ...
    void closeStreamNoStatusCloseDoesNotReconnect() throws Exception {
        ...
        V2WebSocketClient ws = client.listen().v2().v2WebSocket();
        CountDownLatch disconnected = new CountDownLatch(1);
        ws.reconnectOptions(reconnectOptions());
    ...
    void rejectedCloseStreamDoesNotSuppressReconnect() throws Exception {
        ...
        V2WebSocketClient ws = new V2WebSocketClient(ClientOptions.builder()
