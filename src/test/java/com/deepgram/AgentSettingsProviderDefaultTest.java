...
/**
 * Guards the union default-variant patch on the agent listen-provider unions (see .fernignore):
 * {@code AgentV1SettingsAgentListenProvider}, {@code AgentV1SettingsAgentContextListenProvider}, and
 * {@code AgentV1UpdateListenListenProvider}.
 *
 ...
 * {@code @JsonCreator} has an empty body, so such a payload deserializes to an unknown variant
 * carrying {@code null}: {@code getV2()} comes back empty and re-serializing emits {@code null},
 * silently dropping the caller's provider. All three are patched to {@code defaultImpl = V2Value},
 * which matches the server's default.
 *
 * <p>The two Settings unions shipped in 0.7.1 with the defect, so this is a pre-existing bug fix
 * rather than a regression guard — but it is the same trade-off, so it is pinned the same way.
 */
