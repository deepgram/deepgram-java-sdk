...
import resources.listen.v2.types.ListenV2Configure;
import resources.listen.v2.types.ListenV2ConfigureFailure;
import resources.listen.v2.types.ListenV2ConfigureSuccess;
import resources.listen.v2.types.ListenV2Connected;
import resources.listen.v2.types.ListenV2FatalError;
import resources.listen.v2.types.ListenV2TurnInfo;

/**
 * WebSocket client for the v2 channel. Provides real-time bidirectional communication with
...
public class V2WebSocketClient implements AutoCloseable {
  ...
  public CompletableFuture<Void> connect(V2ConnectOptions options) {
    ...
    if (options.getEagerEotThreshold() != null && options.getEagerEotThreshold().isPresent()) {
      urlBuilder.addQueryParameter(
          "eager_eot_threshold", String.valueOf(options.getEagerEotThreshold().get()));
    }
   ...
   * @return a CompletableFuture that completes when the message is sent
   */
  public CompletableFuture<Void> sendListenV2Configure(ListenV2Configure message) {
    return sendMessage(message);
  }
      ...
      switch (type) {
        case "Connected":
          if (connectedHandler != null) {
            ListenV2Connected event = objectMapper.treeToValue(node, ListenV2Connected.class);
            if (event != null) {
              connectedHandler.accept(event);
            }
          }
          break;
        case "TurnInfo":
        ...
        case "ConfigureSuccess":
          if (configureSuccessHandler != null) {
            ListenV2ConfigureSuccess event =
                objectMapper.treeToValue(node, ListenV2ConfigureSuccess.class);
            if (event != null) {
              configureSuccessHandler.accept(event);
            }
          }
          break;
        case "ConfigureFailure":
          if (configureFailureHandler != null) {
            ListenV2ConfigureFailure event =
                objectMapper.treeToValue(node, ListenV2ConfigureFailure.class);
            if (event != null) {
        ...
        case "Error":
          if (errorHandler != null) {
            ListenV2FatalError event = objectMapper.treeToValue(node, ListenV2FatalError.class);
            if (event != null) {
