...
/**
 * WebSocketListener with automatic reconnection, exponential backoff, and message queuing. Provides
 * production-ready resilience for WebSocket connections.
 */
public abstract class ReconnectingWebSocketListener extends WebSocketListener {
  ...
  private final ConcurrentLinkedQueue<ByteString> binaryMessageQueue =
   ...
   *
   * <p>Connection behavior: - Times out after 4000 milliseconds - Thread-safe via atomic lock
   * (returns immediately if connection in progress) - Retry count not incremented for initial
   * connection attempt
   *
   * <p>Error handling: - TimeoutException: Includes retry attempt context - InterruptedException:
   * Preserves thread interruption status - ExecutionException: Extracts actual cause and adds
   * context
   */
  public void connect() {
    ...
    try {
      CompletableFuture<? extends WebSocket> connectionFuture =
          CompletableFuture.supplyAsync(connectionSupplier);
      try {
        ...
        InterruptedException interruptError =
   ...
   *
   * <p>This method: - Disables automatic reconnection - Clears queued messages to prevent stale
   * data - Closes the WebSocket with standard close code 1000 - Properly shuts down the reconnect
   * executor to prevent thread leaks - Waits up to 5 seconds for executor termination
   */
   ...
   *
   * <p>Thread-safe: Synchronized to prevent race conditions with flushMessageQueue().
   *
   * <p>Behavior: - If connected: Attempts direct send, queues if buffer full - If disconnected:
   * Queues message up to maxEnqueuedMessages limit - If queue full: Message is dropped
   *
   ...
   *
   * <p>Behavior: - If connected: Attempts direct send, queues if buffer full - If disconnected:
   * Queues data up to maxEnqueuedMessages limit - If queue full: Data is dropped
   *
  ...
  /**
   * Gets the current WebSocket instance. Thread-safe method to access the WebSocket connection.
   *
  ...
  public void onFailure(WebSocket webSocket, Throwable t, Response response) {
    ...
    if (t != null) {
      ...
      enhancedError =
          new RuntimeException(
              errorContext + ": " + t.getClass().getSimpleName() + ": " + t.getMessage());
   ...
   *
   * <p>Uses 0-based retry count where: - 0 = initial connection (not used by this method) - 1 =
   * first retry (returns minReconnectionDelayMs) - 2+ = exponential backoff up to
   * maxReconnectionDelayMs
   */
  private long getNextDelay() {
    ...
    long delay =
        (long)
            (minReconnectionDelayMs * Math.pow(reconnectionDelayGrowFactor, retryCount.get() - 1));
    return Math.min(delay, maxReconnectionDelayMs);
  }

  /**
   * Schedules a reconnection attempt with appropriate delay. Increments retry count and uses
   * exponential backoff.
   */
   ...
   *
   * <p>Thread-safe: Synchronized to prevent race conditions with send() method.
   *
   * <p>Algorithm: 1. Drains queue into temporary list to avoid holding lock during sends 2.
   * Attempts to send each message in order 3. If any send fails, re-queues that message and all
   * subsequent messages 4. Preserves message ordering during re-queueing 5. Repeats for binary
   * message queue
   */
  ...
  public static final class ReconnectOptions {
    ...
    public static final class Builder {
       ...
       *
       * <p>Validates that: - All delay values are positive - minReconnectionDelayMs <=
       * maxReconnectionDelayMs - reconnectionDelayGrowFactor >= 1.0 - maxRetries and
       * maxEnqueuedMessages are non-negative
       *
