...
/**
 * The {@code Stream} class implements {@link Iterable} to provide a simple mechanism for reading
 * and parsing objects of a given type from data streamed via a {@link Reader} using a specified
 * delimiter.
 *
 * <p>{@code Stream} assumes that data is being pushed to the provided {@link Reader} asynchronously
 * and utilizes a {@code Scanner} to block during iteration if the next object is not available.
 * Iterable stream for parsing JSON and Server-Sent Events (SSE) data. Supports both
 * newline-delimited JSON and SSE with optional stream termination.
 *
...
public final class Stream<T> implements Iterable<T>, Closeable {
  ...
  public static <T> Stream<T> fromSse(
      Class<T> valueType, Reader sseReader, String streamTerminator) {
    return new Stream<>(valueType, StreamType.SSE, sseReader, streamTerminator);
  }

  /**
   * Creates a stream from SSE data with event-level discrimination support. Use this when the SSE
   * payload is a discriminated union where the discriminator is an SSE envelope field (e.g.,
   * 'event').
   *
   * @param valueType The class of the objects in the stream.
  ...
  public static <T> Stream<T> fromSseWithEventDiscrimination(
    ...
    return new Stream<>(
        valueType, StreamType.SSE_EVENT_DISCRIMINATED, sseReader, null, discriminatorProperty);
  }
  ...
  /**
   * Returns an iterator over the elements in this stream that blocks during iteration when the next
  ...
  private final class JsonIterator implements Iterator<T> {
     ...
     *
     * <p>Will block and wait for input if the stream has not ended and the next object is not yet
     * available.
     *
    ...
    public T next() {
      ...
      if (!scanner.hasNext()) {
        ...
        try {
          T parsedResponse = ObjectMappers.JSON_MAPPER.readValue(scanner.next().trim(), valueType);
        ...
        while (sseScanner.hasNextLine()) {
          ...
          if (line.trim().isEmpty()) {
            if (eventDataBuffer.length() > 0) {
              try {
                nextItem =
                    ObjectMappers.JSON_MAPPER.readValue(eventDataBuffer.toString(), valueType);
                hasNextItem = true;
  ...
  /**
   * Iterator for SSE streams with event-level discrimination. Uses SseEventParser to construct the
   * full SSE envelope for Jackson deserialization.
   */
          ...
          if (line.trim().isEmpty()) {
            if (eventDataBuffer.length() > 0 || currentEventType != null) {
              try {
                ...
                nextItem =
                    SseEventParser.parseEventLevelUnion(
                        currentEventType,
